...
export default {
  ...
  mounted() {
    this.gameName = VueCookies.get("dataR");
    if (this.gameName !== null) {
      let gameName = this.gameName;
      axios
        .get("http://localhost:8000/" + gameName + "/", {
          headers: {},
        })
        .then((response) => {
    ...
    VueCookies.set("dataR", "");
  },
  methods: {
    say(id, title) {
      let url = "http://localhost:8000/game/" + id;
      let post_url = "http://localhost:8000/add_game/";
      let game = confirm(
        `Adicionar o jogo (${id}) - ${title} na sua lista de jogos?`
      );
      if (game) {
        axios
          .get(url, {
